...
class ReActAgent(BaseAgent):
    ...
    def __init__(
        ...
        self._except_thought = except_thought
    ...
    def remove_last_step(self) -> None:
        chat_memory = self._memory.get_all()
        idx = len(chat_memory) - 1 
        while idx>0:
            if chat_memory[idx].role == "user":
                break
            idx -= 1
        self._memory.set(chat_memory[:idx])
    
    def update_last_step(self, text: str) -> None:
        chat_memory = self._memory.get_all()
        idx = len(chat_memory) - 1
        while idx>0:
            if chat_memory[idx].role == "user":
        ...
        self._memory.update_last(text)

    def fetch_last_step(self) -> ChatMessage:
        chat_memory = self._memory.get_all()
        idx = len(chat_memory) - 1
        while idx>0:
            if chat_memory[idx].role == "user":
    ...
    def _extract_reasoning_step(
        ...
        current_reasoning = []
        try:
            reasoning_step = self._output_parser.parse(message_content, is_streaming, except_thought=self._except_thought)
    ...
    def _process_actions(
        ...
        output: ChatResponse,
        previous_reasoning: Optional[List[BaseReasoningStep]] = None,
        ...
        with self.callback_manager.event(
            ...
            previous_conversation = "\n".join([str(msg) for msg in self._memory.get_all()]) + "\n".join([prev_conv.get_content() for prev_conv in previous_reasoning])
            tool_output = tool.call(**reasoning_step.action_input, conversation=previous_conversation)
    ...
    async def _aprocess_actions(
        ...
        with self.callback_manager.event(
            ...
            previous_conversation = "\n".join([str(msg) for msg in self._memory.get_all()]) + "\n".join([prev_conv.get_content() for prev_conv in previous_reasoning])
            tool_output = await tool.acall(**reasoning_step.action_input, conversation=previous_conversation)
            event.on_end(payload={EventPayload.FUNCTION_OUTPUT: str(tool_output)})
    ...
    def chat(
        ...
        for _ in range(self._max_iterations):
            ...
            reasoning_steps, is_done = self._process_actions(
                tools, output=chat_response, previous_reasoning=current_reasoning
            )
    ...
    def stream_chat(
        ...
        while (not is_done) and (ix < self._max_iterations):
            ...
            reasoning_steps, _ = self._process_actions(
                tools=tools, output=full_response, previous_reasoning=current_reasoning, is_streaming=True
            )
            current_reasoning.extend(reasoning_steps)
